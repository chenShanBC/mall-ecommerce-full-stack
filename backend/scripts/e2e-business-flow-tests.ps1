...
function New-DefaultReportPath {
    $dir = Join-Path $PSScriptRoot "..\test-reports"
    if (-not (Test-Path $dir)) { New-Item -ItemType Directory -Force -Path $dir | Out-Null }
    return (Join-Path $dir ("e2e-business-flow-{0}.md" -f (Get-Date -Format "yyyyMMdd-HHmmss")))
}

if ([string]::IsNullOrWhiteSpace($ReportPath)) {
    $ReportPath = New-DefaultReportPath
}
...
function Invoke-Case {
    ...
    try {
        ...
        Add-TestResult -Suite $Suite -Name $Name -Status "FAIL" -Detail $detail -ElapsedMs $sw.ElapsedMilliseconds
        Write-Host "[FAIL] $Suite - $Name : $detail" -ForegroundColor Red
...
function Run-SmokeSuite {
    Write-Section "Smoke: auth, product, current user"

    Invoke-Case -Suite "Smoke" -Name "user login and current user" -Script {
        $token = Ensure-UserToken
        $me = Invoke-MallApi -Method GET -Path "/api/users/me" -Token $token
        $data = Get-ResponseData $me
        "userId=$($data.id)"
    } | Out-Null

    Invoke-Case -Suite "Smoke" -Name "admin login and current admin" -Script {
        $token = Ensure-AdminToken
        $me = Invoke-MallApi -Method GET -Path "/api/admin/me" -Token $token
        $data = Get-ResponseData $me
        "admin=$($data.username)"
    } | Out-Null

    Invoke-Case -Suite "Smoke" -Name "category and product detail are reachable" -Script {
        Invoke-MallApi -Method GET -Path "/api/categories" | Out-Null
        $page = Invoke-MallApi -Method GET -Path "/api/products?page=1&size=10"
        $product = Get-FirstArrayItem (Get-ResponseData $page)
        if ($product -and $product.id) {
            $script:Context.productId = $product.id
            Invoke-MallApi -Method GET -Path "/api/products/$($product.id)" | Out-Null
        }
        Invoke-MallApi -Method GET -Path "/api/products/$SkuId" -AllowFailure | Out-Null
        "skuId=$SkuId productSample=$($product.id)"
    } | Out-Null
}

function Run-OrderPaySuite {
    Write-Section "Order pay flow: create order, pay, mock callback, query"

    Invoke-Case -Suite "OrderPayFlow" -Name "create direct order" -Script {
        $token = Ensure-UserToken
    ...
    Invoke-Case -Suite "OrderPayFlow" -Name "create pay order" -Script {
        $token = Ensure-UserToken
    ...
    Invoke-Case -Suite "OrderPayFlow" -Name "mock pay success and query pay order" -Script {
        $token = Ensure-UserToken
        $orderNo = $script:Context.orderNo
    ...
    Invoke-Case -Suite "OrderPayFlow" -Name "user orders list and detail are reachable" -Script {
        $token = Ensure-UserToken
...
function Run-InternalStockSuite {
...
function Run-AdminSuite {
    Write-Section "Admin flow: order, pay, reconciliation, aftersale"

    Invoke-Case -Suite "AdminFlow" -Name "admin query orders and pays" -Script {
        $token = Ensure-AdminToken
        $orderNo = $script:Context.orderNo
    ...
    Invoke-Case -Suite "AdminFlow" -Name "admin reconciliation overview is reachable" -Script {
        $token = Ensure-AdminToken
    ...
    Invoke-Case -Suite "AdminFlow" -Name "admin aftersales list is reachable" -Script {
        $token = Ensure-AdminToken
    ...
    if ($script:Context.orderNo) {
        Invoke-Case -Suite "AdminFlow" -Name "admin order status transition can be called" -Script {
            $token = Ensure-AdminToken
            $orderNo = $script:Context.orderNo
...
function Write-Report {
    ...
    $lines.Add("# mallFei E2E Test Report") | Out-Null
    $lines.Add("") | Out-Null
...
Run-OrderPaySuite
Run-AdminSuite
Run-AddressCartSuite
Run-AdminP1Suite
Run-InternalStockSuite
Write-Report

if (($script:Results | Where-Object { $_.status -eq 'FAIL' }).Count -gt 0) {
    throw 'There are failed cases. Please check the report.'
}

Write-Host "All automated test cases completed" -ForegroundColor Green

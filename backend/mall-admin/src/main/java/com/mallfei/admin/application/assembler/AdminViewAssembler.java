...
public class AdminViewAssembler {
    ...
    public AdminOrderSummaryView toOrderSummary(Order order) { return new AdminOrderSummaryView(order.id(), order.orderNo(), order.userId(), order.orderStatus(), order.totalAmountCent(), order.payAmountCent(), order.receiverName(), order.receiverPhone(), order.itemCount()); }
    public AdminOrderDetailView toOrderDetail(Order order) { return new AdminOrderDetailView(order.id(), order.orderNo(), order.userId(), order.orderStatus(), order.totalAmountCent(), order.payAmountCent(), order.receiverName(), order.itemCount(), order.receiverPhone(), text(order.receiverProvinceName()) + text(order.receiverCityName()) + text(order.receiverDistrictName()) + text(order.receiverDetailAddress()), order.remark(), order.paidAt(), order.cancelledAt(), order.shippedAt(), order.completedAt(), order.items().stream().map(item -> new AdminOrderItemView(item.id(), item.skuId(), item.skuName(), item.quantity(), item.salePriceCent(), item.totalAmountCent())).toList()); }
    public AdminPaySummaryView toPaySummary(PayOrder payOrder) { return new AdminPaySummaryView(payOrder.id(), payOrder.payOrderNo(), payOrder.orderNo(), payOrder.userId(), payOrder.payStatus(), payOrder.payAmountCent(), payOrder.payChannel()); }
    ...
    public AdminDashboardStatsView toDashboardStats(List<Order> orders, List<PayOrder> pays) { return new AdminDashboardStatsView(orders.size(), countOrders(orders, Order.STATUS_PENDING_PAYMENT), countOrders(orders, Order.STATUS_PAID), countOrders(orders, Order.STATUS_SHIPPED), countOrders(orders, Order.STATUS_COMPLETED), orders.stream().filter(Order::cancelled).count(), orders.stream().filter(Order::paidOrAfter).mapToLong(Order::safePayAmountCent).sum(), pays.size(), pays.stream().filter(PayOrder::pending).count(), pays.stream().filter(PayOrder::success).count(), pays.stream().filter(PayOrder::closed).count()); }
    ...
    public AdminDashboardOverviewView toDashboardOverview(AdminDashboardStatsView stats, AdminStockWarningStatsView stockWarningStats, long abnormalReconcileCount, List<AdminDashboardTodoView> todos, List<AdminDashboardShortcutView> shortcuts) { return new AdminDashboardOverviewView(stats, stockWarningStats, abnormalReconcileCount, todos, shortcuts); }

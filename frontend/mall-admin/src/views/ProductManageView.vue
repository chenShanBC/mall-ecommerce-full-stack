<template>
  <AdminLayout title="商品管理" @refresh="loadProducts" @logout="handleLogout">
    ...
    <el-dialog draggable v-model="detailVisible" title="商品详情" width="900px" append-to-body destroy-on-close align-center>
      <div v-if="productDetail" class="admin-detail">
        ...
        <el-table :data="productDetail.skus || []" class="admin-table">
          ...
          <el-table-column label="销量" width="90"><template #default="{ row }">{{ skuSalesText(row) }}</template></el-table-column>
          <el-table-column label="销售表现" width="118"><template #default="{ row }"><span class="sales-band-pill" :class="`sales-band-pill--${skuSalesBand(row).toLowerCase()}`">{{ skuSalesBandLabel(row) }}</span></template></el-table-column>
...
const routeHasProductFilter = () => Boolean(route.query.keyword || route.query.categoryId || route.query.status || route.query.salesBand || route.query.stockWarningStatus || route.query.hotSalesThreshold || route.query.lowSalesThreshold || route.query.sortBy || route.query.sortOrder || route.query.page || route.query.size);
const initialQueryMode = routeHasProductFilter();
const filters = reactive({ keyword: String(route.query.keyword ?? (initialQueryMode ? '' : '')), categoryId: route.query.categoryId ? Number(route.query.categoryId) : null, status: String(route.query.status ?? ''), salesBand: String(route.query.salesBand ?? ''), stockWarningStatus: String(route.query.stockWarningStatus ?? ''), hotSalesThreshold: normalizeSalesThreshold(route.query.hotSalesThreshold, salesThresholdBase.hotSalesThreshold), lowSalesThreshold: normalizeSalesThreshold(route.query.lowSalesThreshold, salesThresholdBase.lowSalesThreshold) });
const query = reactive({ sortBy: String(route.query.sortBy || 'id'), sortOrder: String(route.query.sortOrder || 'desc') });
const pager = reactive({ page: Number(route.query.page || 1), size: Number(route.query.size || ADMIN_PAGE_SIZE), total: Number(initialQueryMode ? 0 : adminPageCache.products.pager.total || 0) });
...
const applyRouteQuery = () => {
  ...
  filters.stockWarningStatus = String(route.query.stockWarningStatus ?? '');
...
const skuSalesText = (row) => row.monthlySalesCount ?? row.monthly_sales_count ?? row.salesCount ?? row.sales_count ?? row.totalSales ?? row.salesVolume ?? 0;
const skuSalesBand = (row) => String(row.salesBand || row.skuSalesBand || resolveSalesBand(skuSalesText(row))).toUpperCase();
const skuSalesBandLabel = (row) => row.salesBandLabel || row.skuSalesBandLabel || ({ HOT: '热销', LOW: '低销', NORMAL: '常规' }[skuSalesBand(row)] || '常规');

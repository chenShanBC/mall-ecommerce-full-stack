<template>
  <div class="role-panel role-panel--warehouse">
    ...
    <div class="panel-grid panel-grid--2">
      <DashboardDataTable title="风险雷达 / 仓储待办" desc="聚合缺货、低库存、库存对账和锁定库存风险" :columns="riskColumns" :rows="data.risks || []" :loading="loading" @refresh="$emit('refresh')" />
      <DashboardDataTable title="库存预警列表" desc="优先展示需要补货或人工处理的库存风险" :columns="warningColumns" :rows="data.warnings || []" :loading="loading" @refresh="$emit('refresh')" />
    </div>
...
const hasStock = computed(() => [summary.value.normalCount, summary.value.outOfStockCount, summary.value.lowStockCount, summary.value.lockedStockTotal, summary.value.stockDiffCount].some(Boolean));
...
const handleStockDrilldown = (event) => {
  if (event.name === '低库存') go('/stocks', { warningStatus: 'LOW' });
  if (event.name === '高库存') go('/stocks', { warningStatus: 'HIGH' });
  if (event.name === '对账异常') go('/reconciliations');
};
